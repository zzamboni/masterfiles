...
echo '000 Setting hostname to $client_hostname.' >> \$LOG 2>&1
/bin/hostname '$client_hostname'
...
";
 ...
 return $go . "
echo '001 installing cfengine-nova' >> \$LOG 2>&1

curl -o /tmp/cfengine-nova_3.5.2-1_amd64.deb http://`cat /var/tmp/cfhub.ip`/tmp/cfengine-nova_3.5.2-1_amd64.deb >> \$LOG 2>&1
dpkg -i /tmp/cfengine-nova_3.5.2-1_amd64.deb >> \$LOG 2>&1

#curl -o /tmp/cfengine.gpg.key http://cfengine.com/pub/gpg.key >> \$LOG 2>&1

#apt-key add /tmp/cfengine.gpg.key >> \$LOG 2>&1

#echo 'deb http://cfengine.com/pub/apt precise main' >> /etc/apt/sources.list.d/cfengine.list
...
/var/cfengine/bin/cf-agent -B `cat /var/tmp/cfhub.ip` >> $LOG 2>&1
';
}
